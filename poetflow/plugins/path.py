...
class PathRewriter:
    ...
    def execute(self, event: ConsoleCommandEvent):
        ...
        directory_deps = [
            dep for dep in main_deps_group.dependencies if isinstance(dep, DirectoryDependency)
        ]
    ...
    def _get_dependency_pyproject(
        self, poetry: Poetry, dependency: DirectoryDependency
    ) -> PyProjectTOML:
    ...
    def _pin_dependency(self, poetry: Poetry, dependency: DirectoryDependency):
        ...
        if self.plugin_conf.version_rewrite_rule in ["~", "^"]:
            ...
            raise ValueError(
                f"Invalid version rewrite rule: {self.plugin_conf.version_rewrite_rule}"
            )
